...
let nodeUtil = require("util"),
  ...
  path = require("path"),
  _ = require("lodash"),
  PDFParser = require("../pparserequire"),
...
let optimist = require("optimist")
  ...
  .describe(
    "f",
    "(required) Full path of input PDF file or a directory to scan for all PDF files. When specifying a PDF file name, it must end with .PDF, otherwise it would be treated as a input directory.\n"
  )
  .alias("o", "output_dir")
  .describe(
    "o",
    "(optional) Full path of output directory, must already exist. Current JSON file in the output folder will be replaced when file name is same.\n"
  )
  .alias("s", "silent")
  .describe(
    "s",
  ...
  .describe(
    "m",
    "(optional) when specified, will generate .merged.json that includes auto-merged broken text blocks from PDF (Experimental).\n"
  )
  .alias("r", "stream")
  .describe(
    "r",
    "(optional) when specified, will process and parse with buffer/object transform stream rather than file system (Experimental).\n"
  );
...
let PDF2JSONUtil = (function () {
  ...
  let _continue = function (callback, err) {
    ...
    if (nodeUtil.isFunction(callback)) callback(err);
  };
  ...
  let _generateMergedTextBlocksStream = function (callback) {
    let outputStream = _createOutputStream.call(
      this,
  ...
  let _processAdditionalStreams = function (outputTasks, callback) {
    if (PROCESS_FIELDS_CONTENT) {
      ...
      outputTasks.push((cbFunc) =>
        _generateFieldsTypesStream.call(this, cbFunc)
      );
    }
    if (PROCESS_RAW_TEXT_CONTENT) {
      //needs to generate content.txt file
      outputTasks.push((cbFunc) =>
        _generateRawTextContentStream.call(this, cbFunc)
      );
    }
    if (PROCESS_MERGE_BROKEN_TEXT_BLOCKS) {
      //needs to generate json file with merged broken text blocks
      outputTasks.push((cbFunc) =>
        _generateMergedTextBlocksStream.call(this, cbFunc)
      );
    }
  ...
  let _onPrimarySuccess = function (callback) {
    console.log(
      "SUCCESS: [" + this.inputPath + "] => [" + this.outputPath + "]"
    );
  ...
  let _parseOnePDFStream = function (callback) {
    ...
    this.pdfParser.on("pdfParser_dataError", (evtData) =>
      _onPdfParserError.call(this, evtData, callback)
    );
    ...
    let inputStream = fs.createReadStream(this.inputPath, {
      bufferSize: 64 * 1024,
    });
    inputStream
      .pipe(this.pdfParser)
...
let PDFProcessor = (function () {
  ...
  cls.prototype.processFiles = function (inputDir, files) {
    ...
    this.p2j.processFile(
      function processPDFFile(err) {
        if (err) {
          this.complete(err);
        } else {
          fId++;
  ...
  cls.prototype.processOneDirectory = function () {
    ...
    fs.readdir(inputDir, (err, files) => {
      ...
      let pdfFiles = files.filter(
        (file) =>
          file.substr(-4).toLowerCase() === ".pdf" &&
          _iChars.indexOf(file.substr(0, 1)) < 0
      );
